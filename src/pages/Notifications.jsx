...
const Notifications = ({ user, onNavigateToExperience }) => {
  ...
  if (error) return (
    <div className="notifications-error">
      <div className="notifications-error-icon">{getErrorMessage()}</div>
      <p>{error}</p>
      {(errorType === 'session_expired' || errorType === 'login_required') && (
        <button className="notifications-error-btn" onClick={() => window.location.href = '/login'}>
          {t('auth.login')}
        </button>
      )}
      {(errorType === 'load_error' || errorType === 'server_error') && (
        <button className="notifications-error-btn" onClick={() => { setError(null); setErrorType(null); fetchNotifications(); }}>
            ...
            return (
              ...
              >
                <div className="notification-avatar">
                  {data.actor_avatar ? (
                    <img src={data.actor_avatar} alt={data.actor_name || t('notifications.defaultAvatar')}
                      onError={(e) => { e.target.onerror = null; e.target.src = "https://via.placeholder.com/120"; }} />
                  ) : (
                    <div className="avatar-placeholder">{data.actor_name?.charAt(0) || '?'}</div>
                  )}
                </div>
